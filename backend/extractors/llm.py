...
DŮLEŽITÉ PRO ČÁSTKY:
- České částky často obsahují mezery jako oddělovače tisíců: "44 413,00" = 44413.00
- Pozor na chyby OCR: "4413" může být "44 413" (chybí mezera)
- Pokud vidíš podezřele malou částku, zkus najít větší částku v textu
- Kontroluj konzistenci mezi částkami bez DPH, DPH a s DPH
- Běžné OCR chyby: O/o → 0, l/I/| → 1, S/s → 5, B → 8, G → 6, Z/z → 2

DŮLEŽITÉ PRO ČESKÝ TEXT - OPRAVUJ TYTO CHYBY:
- "Komeréni" → "Komerční" (chybí č)
- "Pizen" → "Plzeň" (chybí ř)
- "banka" → "banka" (kontroluj kontext)
- Opravuj všechny zjevné OCR chyby v českých slovech

ČESKÉ ZNAKY: č, ř, ž, š, ě, á, é, í, ó, ú, ů, ý
...
def extract_fields_llm(text: str) -> dict:
    ...
    # Vylepšené parametry pro přesnější extrakci
    resp = client.chat.completions.create(
        model=model,
        messages=[
            {"role": "system", "content": "You are a precise information extraction assistant specialized in Czech invoices. Always return valid JSON and pay special attention to Czech number formats and text."},
            {"role": "user", "content": _prompt(text)},
        ],
        temperature=0.1,  # Nižší teplota pro konzistentnější výsledky
        max_tokens=2000,  # Více tokenů pro detailnější odpověď
        top_p=0.9,  # Lepší kontrolu nad generováním
    )
