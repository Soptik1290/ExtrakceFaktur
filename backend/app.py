...
async def extract(file: UploadFile = File(...), method: Optional[str] = Query("auto")):
    try:
        ...
        # Log pro debugging
        print(f"Extrahovaný text ({len(text)} znaků): {text[:200]}...")
        
        # Detekce chybných částek
        from .extractors.utils import _detect_amount_errors
        amount_errors = _detect_amount_errors(text)
        if amount_errors:
            print(f"Detekovány problémy s částkami: {amount_errors}")
        ...
        if result is None and (method == "llm" or (method == "auto" and llm_available())):
            try:
                ...
                print(f"LLM extrakce selhala: {e}")
                result = None
        ...
        if isinstance(result, dict):
            ...
            # Log výsledků
            print(f"Výsledek extrakce: {result.get('castka_s_dph')} CZK")
        else:
            result = {}
